...
#if defined(NDEBUG)
# error "Cafecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x42d88fb84919e81ce41a6c21f2d4a78893aa1d5055eb3ae42fe5d68d610bae40");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Cafecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Cafecoin Signed Message:\n";
...
{
    // Cafecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Cafecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Cafecoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Cafecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Cafecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Cafecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc8d4eaeeedec293602f9a2489da896dab0afa637289940253003d986ab0cddfe");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "My first Cafe Coin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b528106349ddaa986d08d17d4852ac3388e868b97e2ff91f699f262ee7ae3d2f816bf1cb641a41eff251df16a7a40a0e40d798a50e74897b7ea0a816bb8ad5d3") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1534825118;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085839616;

        if (fTestNet)
        {
            block.nTime    = 1534825118;
            block.nNonce   = 386925390;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x02d9a601e6891d72fee365c97313220c4a9493c27f94dcd1c213cfd1451f48ef"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xb8, 0xdd }; // Cafecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CafecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CafecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CafecoinMiner(CWallet *pwallet)
{
    printf("CafecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cafecoin-miner");
    ...
    try { loop {
        ...
        printf("Running CafecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CafecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CafecoinMiner, pwallet));
}
